...
// This will hold the CPU registers
// Because if we decide making it multi-tasking we will need to be able to flik between them
struct registers
{
    uint32_t edi;
    uint32_t esi;
    ...
    uint32_t ip; // command that was last executed
    uint32_t cs;
    uint32_t flags;
    uint32_t esp;
    uint32_t ss;
