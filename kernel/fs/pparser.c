...
{
    int len = strnlen(filename, POTONGOS_MAX_PATH);
...
{
    char *result_path_part = kzalloc(POTONGOS_MAX_PATH);
    int i = 0;
...
struct path_root *pathparser_parse(const char *path, const char *current_directory_path)
{
    ...
    if (strlen(path) > POTONGOS_MAX_PATH)
    {
        goto out;
    }
...
Test:
    1. gdb
    2. add-symbol-file ../build/fs/kernelfull.o 0x100000
    3. break kernel.c:104
    4. target remote | qemu-system-i386 -hda ./os.bin -S -gdb stdio

    5. c - to continue
    6. next
    7. print root_path
    7.1. print *root_path
    7.2. print *root_path->first
    7.3. print *root_path->next
